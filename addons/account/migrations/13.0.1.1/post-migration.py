...
def migration_invoice_moves(env):
    ...
    openupgrade.logged_query(
        ...
        WHERE am.id = aml.move_id AND ail.company_id = aml.company_id AND ail.account_id = aml.account_id
            ...
            AND ((ail.account_analytic_id IS NULL AND aml.analytic_account_id IS NULL) OR ail.account_analytic_id = aml.analytic_account_id)
        RETURNING aml.id""",
    )
    ...
    openupgrade.logged_query(
        ...
        WHERE am.id = aml.move_id AND ait.company_id = aml.company_id AND ait.account_id = aml.account_id
            ...
            AND ((ait.account_analytic_id IS NULL AND aml.analytic_account_id IS NULL) OR ait.account_analytic_id = aml.analytic_account_id)""",
    )
